...
// ðŸ”¥ Duplicate category fix
$check = mysqli_query($conn, "SELECT * FROM audios WHERE LOWER(category)=LOWER('$category')");
if(mysqli_num_rows($check) > 0){
    $row = mysqli_fetch_assoc($check);
    $category = $row['category']; // same existing category use hogi
}

// âœ… folder name same rakho
...
// âœ… YAHAN FIX
$folder = "audios/" . $category_folder . "/" . $file_name;

// Folder create if not exists
if(!is_dir("audios/" . $category_folder)){
    mkdir("audios/" . $category_folder, 0777, true);
}
...
// Announcement Data
$announcement_title = "New Audio Uploaded!";
$announcement_desc = "A new audio lecture titled \"$title_safe\" has been added in $category_safe category.";

// INSERT QUERY
$announcement_query = "INSERT INTO announcements 
(title, description, category, added_by, views, is_pinned, created_at) 
VALUES 
('$announcement_title', '$announcement_desc', 'audio', 'Admin', 0, 0, NOW())";
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input list="categories" name="category" class="form-input" placeholder="e.g. C Language, Java" required>

<datalist id="categories">
<?php
$res = mysqli_query($conn,"SELECT DISTINCT category FROM audios");
...
?>
    ...
    Swal.fire({
        icon: 'success',
        title: 'ðŸŽ§ Uploaded Successfully!',
