...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        /* ===== SEARCH BAR STYLES (Only New Addition) ===== */
        .search-section {
            padding: 35px 0;
            background: #fff;
        ...
        .header-area{background:#fff;box-shadow:0 2px 15px rgba(0,0,0,0.1);padding:18px 0;position:sticky;top:0;z-index:1000;transition:box-shadow 0.3s ease}
...
<div class="search-section">
    <div class="search-container">
        ...
        <input type="text" id="notesSearch" class="search-input" 
               placeholder="ðŸ” Search notes, papers or syllabus by title...">
    </div>
</div>
...
<ul>
<?php
$result = getData($conn, 'Pharmacy', '1', 'notes','1');
while($row = mysqli_fetch_assoc($result)){
echo "<li><a href='{$row['file_path']}' download>{$row['title']}</a></li>";
}
?>
...
<ul>
<?php
$result = getData($conn, 'Pharmacy', '1', 'paper','1st_year');
while($row = mysqli_fetch_assoc($result)){
echo "<li><a href='{$row['file_path']}' download>{$row['title']}</a></li>";
}
?>
...
<ul>
<?php
$result = getData($conn, 'Pharmacy', '1', 'syllabus','1st_year');
while($row = mysqli_fetch_assoc($result)){
echo "<li><a href='{$row['file_path']}' download>{$row['title']}</a></li>";
}
?>
...
<ul>
<?php
$result = getData($conn, 'Pharmacy', '2', 'notes','2nd_year');
while($row = mysqli_fetch_assoc($result)){
echo "<li><a href='{$row['file_path']}' download>{$row['title']}</a></li>";
}
?>
...
<ul>
<?php
$result = getData($conn, 'Pharmacy', '2', 'paper','2nd_year');
while($row = mysqli_fetch_assoc($result)){
echo "<li><a href='{$row['file_path']}' download>{$row['title']}</a></li>";
}
?>
...
<ul>
<?php
$result = getData($conn, 'Pharmacy', '2', 'syllabus','2nd_year');
while($row = mysqli_fetch_assoc($result)){
echo "<li><a href='{$row['file_path']}' download>{$row['title']}</a></li>";
}
?>
...
// REVEAL ANIMATION & Navbar scroll (unchanged)
function reveal(){
